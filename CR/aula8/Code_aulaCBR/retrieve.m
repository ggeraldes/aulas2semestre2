function [retrieved_indexes, similarities, new_case] = retrieve(case_library, new_case, threshold)
    ...
    for i=1:size(case_library,1)
        ...
        final_similarity = 1.0 - sum(distances*weighting_factors)/sum(weighting_factors);
...
function [holiday_type_sim] = get_holiday_type_similarities()
    ...
    holiday_type_sim.similarities = [
        % Active Bathing City Education Language Recreation Skiing Wandering
          1.0 .2 .7 .8 .6 .4 .9 .8                                   % Active
          .4 1.0 .7 .8 .6 .4 .9 .8                                   % Bathing
          .4 .2 1.0 .8 .6 .4 .9 .8                                   % City
          .4 .2 .7 1.0 .6 .4 .9 .8                                 % Education
          .4 .2 .7 .8 1.0 .4 .9 .8                                   % Language
          .4 .2 .7 .8 .6 1.0 .9 .8                                   % Recreation
          .4 .2 .7 .8 .6 .4 1.0 .8                                   % Skiing
          .4 .2 .7 .8 .6 .4 .9 1.0                                   % Wandering
    ];
end
...
function [accommodation_sim] = get_accommodation_similarities()
    ...
    accommodation_sim.similarities = [
        ...
        1 0.3 0.4 0.5 0.6 0.7                                                             % FiveStars
        0.3 1 0.4 0.5 0.6 0.7                                                             % FourStars
        0.4 0.4 1 0.5 0.6 0.7                                                              % HolidayFlat
        0.5 0.5 0.5 1 0.6 0.7                                                              % OneStar
        0.6 0.6 0.6 0.6 1 0.7                                                              % ThreeStars
        0.7 0.7 0.7 0.7 0.7 1                                                              % TwoStars
    ];
end
